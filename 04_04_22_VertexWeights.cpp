...
class VertexEdgeWeightedDigraph {
private:
...
public:
    ...
    VertexEdgeWeightedDigraph( int V )
        : V( V ),
          adj( V ),
          vertexWeights( V, numeric_limits< double >::infinity() )
    {
        // not implemented
    }

    virtual ~VertexEdgeWeightedDigraph() {
        // not implemented
    }
    ...
    void addVertexWeight( int v, double weight ) {
        vertexWeights[ v ] = weight;
    }

    double getVertexWeight( int v ) const {
    ...
    friend ostream& operator <<( ostream& out, const VertexEdgeWeightedDigraph& G ) {
...
private:
    ...
    void relax( const VertexEdgeWeightedDigraph& G, int v ) {
...
public:
    ...
    DijkstraSP( const VertexEdgeWeightedDigraph& G, int s )
        : edgeTo( G.getV() ),
          distTo( G.getV(), numeric_limits< double >::infinity() ),
          indexMinPQ( G.getV() )
    {
        distTo[ s ] = G.getVertexWeight( s );
        indexMinPQ.insert( s, G.getVertexWeight( s ) );
...
int main( int argc, char ** argv ) {
    ...
    double graphVertexWeightsArr[ 8 ] {
        0.58, 0.32, 0.26, 0.29, 0.93, 0.28, 0.52, 0.34
    };

    VertexEdgeWeightedDigraph ewDigraph( 8 );

    for ( int i = 0; i < 8; ++i ) {
        ewDigraph.addVertexWeight( i, graphVertexWeightsArr[ i ] );
    }

    for ( int i = 0; i < 15; ++i ) {
        DirectedEdge e( (int)graphArr[ i ][ 0 ], (int)graphArr[ i ][ 1 ], graphVertexWeightsArr[ graphArr[ i ][ 1 ] ] );
        ewDigraph.addEdge( e );
    }
