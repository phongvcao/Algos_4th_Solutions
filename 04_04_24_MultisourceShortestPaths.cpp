...
public:
    ...
    DijkstraSP( const EdgeWeightedDigraph& G, const vector< int >& sources )
        : distTo( G.getV() + 1, numeric_limits< double >::infinity() ),
          edgeTo( G.getV() + 1 ),
          indexMinPQ( G.getV() + 1 )
    {
        EdgeWeightedDigraph newGraph( G.getV() + 1 );
        for ( DirectedEdge e : G.getEdges() ) {
            newGraph.addEdge( e );
        }

        int dummyVertex = G.getV();
        for ( int s : sources ) {
            DirectedEdge e( dummyVertex, s, 0 );
            newGraph.addEdge( e );
        }

        distTo[ dummyVertex ] = 0;
        indexMinPQ.insert( dummyVertex, 0 );
...
int main( int argc, char ** argv ) {
    ...
    DijkstraSP sp( ewDigraph, vector< int >( { 0, 1, 2, 3, 4 } ) );
    cout << "DEBUG: sp: \n" << sp << endl;
