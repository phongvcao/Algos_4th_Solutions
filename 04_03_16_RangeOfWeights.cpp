...
class RangeOfWeights {
private:

    vector< bool > marked;
    vector< Edge > edgeTo;
    priority_queue< Edge, vector< Edge >, greater< Edge > > cycle;
    Edge newEdge;
...
public:
    ...
    RangeOfWeights( queue< Edge > mst, int V, const Edge& e )
        : marked( V, false ),
          edgeTo( V ),
          newEdge( e )
    {
        ...
        auto pq = getCycle();
        min = pq.top().getWeight();
        while ( ! pq.empty() ) {
            if ( max == numeric_limits< double >::infinity() || max < pq.top().getWeight() ) {
                max = pq.top().getWeight();
            }
            pq.pop();
        }
    }

    virtual ~RangeOfWeights() {
        // not implemented
    }
        ...
        for ( Edge e : G.getAdj( v ) ) {
            ...
            if ( hasCycle() ) {
                ...
                for ( int x = v; x != w; x = edgeTo[ x ].getOther( x ) ) {
                    if ( isSame( newEdge, edgeTo[ x ] ) ) {
                        cycle.push( edgeTo[ x ] );
                    }
                }

                if ( isNotSame( newEdge, e ) ) {
                    cycle.push( e );
                }
            }
        }
    ...
    priority_queue< Edge, vector< Edge >, greater< Edge > > getCycle() const {
        return cycle;
    }
    ...
    friend ostream& operator <<( ostream& out, const RangeOfWeights& rangeOfWeights ) {
        out << "\ncycle:\n";
        auto q = rangeOfWeights.getCycle();
        while ( ! q.empty() ) {
            out << q.top() << ", ";
            q.pop();
        }
...
int main( int argc, char ** argv ) {
    ...
    Edge e( 1, 3, 0.29 );
    RangeOfWeights rangeOfWeights( mst2.getEdges(), ewGraph.getV(), e );
    cout << "DEBUG: RangeOfWeights: \n" << rangeOfWeights << endl;
    cout << "DEBUG: RangeOfWeights.getMin(): \n" << rangeOfWeights.getMin() << endl;
    cout << "DEBUG: RangeOfWeights.getMax(): \n" << rangeOfWeights.getMax() << endl;
