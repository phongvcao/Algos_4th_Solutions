    ...
    void relax( const EdgeWeightedDigraph& G, int v ) {
        for ( DirectedEdge e : G.getAdj( v ) ) {
            ...
            if ( distTo[ w ] > distTo[ v ] + e.getWeight() ) {
                ...
                if ( distTo[ w ] <= maxWeight ) {
                    validVertices.insert( w );
                }
            }
        }
    }
...
public:
    ...
    DijkstraNeighborsSP( const EdgeWeightedDigraph& G, int s, double maxWeight )
    ...
    friend ostream& operator <<( ostream& out, const DijkstraNeighborsSP& sp ) {
        out << "\nsp:\n";
        ...
        out << "\n\nvalidVertices:\n";
        for ( int v : sp.validVertices ) {
            out << v << " : " << sp.getDistTo( v ) << endl;
        }
...
int main( int argc, char ** argv ) {
    ...
    DijkstraNeighborsSP sp( ewDigraph, 0, 0.8 );
    cout << "DEBUG: sp: \n" << sp << endl;
