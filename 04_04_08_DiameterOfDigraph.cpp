...
public:
    ...
    Key get( int k ) const {
        if ( ! contains( k ) ) throw runtime_error( "Invalid vertex for priority queue" );
        return keys[ k ];
    }
...
private:
    ...
    double diameter = -1 * numeric_limits< double >::infinity();

public:

    DiameterCalculator( const EdgeWeightedDigraph& G ) {
        for ( int v = 0; v < G.getV(); ++v ) {
            DijkstraSP sp( G, v );
            for ( int w = 0; w < G.getV(); ++w ) {
                if ( sp.getDistTo( w ) > diameter ) {
                    diameter = sp.getDistTo( w );
                }
            }
        }
    }
...
int main( int argc, char ** argv ) {
    ...
    DiameterCalculator dc( ewDigraph );
    cout << "DEBUG: diameter: \n" << dc.getDiameter() << endl;
