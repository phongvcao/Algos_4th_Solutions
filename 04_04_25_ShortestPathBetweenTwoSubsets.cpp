...
public:
    ...
    DijkstraSP( const EdgeWeightedDigraph& G, const vector< int >& sources, const vector< int >& sinks )
        : distTo( G.getV() + 2, numeric_limits< double >::infinity() ),
          edgeTo( G.getV() + 2 ),
          indexMinPQ( G.getV() + 2 )
    {
        EdgeWeightedDigraph newGraph( G.getV() + 2 );
        ...
        int dummyS = G.getV();
        for ( int s : sources ) {
            DirectedEdge e( dummyS, s, 0 );
            newGraph.addEdge( e );
        }

        int dummyT = G.getV() + 1;
        for ( int t : sinks ) {
            DirectedEdge e( t, dummyT, 0 );
            newGraph.addEdge( e );
        }

        // Run DijkstraSP
        distTo[ dummyS ] = 0;
        indexMinPQ.insert( dummyS, 0 );

        while ( ! indexMinPQ.empty() ) {
            relax( newGraph, indexMinPQ.delMin() );
        }
    }
...
int main( int argc, char ** argv ) {
    ...
    DijkstraSP sp( ewDigraph, vector< int >( { 0, 1 } ), vector< int >( { 6, 7 } ) );
    cout << "DEBUG: sp: \n" << sp << endl;
