...
private:
    ...
    int relax( const AdjMatrixEdgeWeightedDigraph& G, int v ) {
        marked[ v ] = true;
        ...
        for ( int w = 0; w < G.getV(); ++w ) {
            if ( ! marked[ w ] && distTo[ w ] < minEdgeWeight ) {
                nextVertex = w;
                minEdgeWeight = distTo[ w ];
            }
        }
        return nextVertex;
    }
...
public:
    ...
    DijkstraDenseGraphSP( const AdjMatrixEdgeWeightedDigraph& G, int s )
          ...
          marked( G.getV(), false )
    {
        ...
        marked[ s ] = true;
        while ( nextVertex != -1 ) {
            nextVertex = relax( G, nextVertex );
        }
    }

    virtual ~DijkstraDenseGraphSP() {
        // not implemented
    }
    ...
    friend ostream& operator <<( ostream& out, const DijkstraDenseGraphSP& sp ) {
        out << "\nsp:\n";
        ...
        for ( int v = 0; v < sp.distTo.size(); ++v ) {
            out << "\n| v : " << v << " : " << sp.distTo[ v ] << " | " << endl;
        }

        out << "\n\nedgeTo:\n";
        for ( int v = 0; v < sp.edgeTo.size(); ++v ) {
            out << "\n| v : " << v << " : " << sp.edgeTo[ v ] << " | " << endl;
        }
...
int main( int argc, char ** argv ) {
    ...
    DijkstraDenseGraphSP sp( ewDigraph, 0 );
    cout << "DEBUG: sp: \n" << sp << endl;
