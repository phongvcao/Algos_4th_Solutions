...
public class BSTPerfectBalance< Key extends Comparable< Key >, Value > {
    ...
    public void insertBalancedly( Key[] keyArr, Value[] valueArr, int low, int high ) {
        if ( low > high ) return;
        int mid = low + ( high - low ) / 2;
        put( keyArr[ mid ], valueArr[ mid ] );
        insertBalancedly( keyArr, valueArr, low, mid - 1 );
        insertBalancedly( keyArr, valueArr, mid + 1, high );
    }

    public static void main( String[] args ) {
        Integer[] keyArr = { 14, 12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9, 11, 13 };
        Integer[] valueArr = { 14, 12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9, 11, 13 };
        BSTPerfectBalance< Integer, Integer > bst = new BSTPerfectBalance< Integer, Integer >();

        quickSort( keyArr );
        quickSort( valueArr );

        bst.insertBalancedly( keyArr, valueArr, 0, keyArr.length - 1 );
