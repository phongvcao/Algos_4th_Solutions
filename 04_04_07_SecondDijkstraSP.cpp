...
private:
    ...
    void relax( const EdgeWeightedDigraph& G, int v ) {
        for ( DirectedEdge e : G.getAdj( v ) ) {
            ...
            } else if ( abs( distTo[ w ] - distTo[ v ] - e.getWeight() ) < numeric_limits< double >::epsilon() ) {
                edgeTo[ w ].push_back( e );
                hasSecondShortestPath = true;
            }
        }
    }
...
public:
    ...
    bool hasPathTo2( int v ) const {
        return hasSecondShortestPath;
    }
    ...
    stack< DirectedEdge > getPathTo2( int v ) const {
        stack< DirectedEdge > path;
        if ( ! hasPathTo( v ) ) return path;
        if ( ! hasPathTo2( v ) ) return path;
        DirectedEdge e = edgeTo[ v ].size() > 1 ? edgeTo[ v ][ 1 ] : edgeTo[ v ][ 0 ];
        while ( e ) {
            path.push( e );
            if ( edgeTo[ e.getFrom() ].size() > 1 ) {
                e = edgeTo[ e.getFrom() ][ 1 ];
            } else {
                e = edgeTo[ e.getFrom() ][ 0 ];
            }
        }
    ...
    friend ostream& operator <<( ostream& out, const DijkstraSP& sp ) {
        ...
        for ( int v = 0; v < sp.edgeTo.size(); ++v ) {
            out << "\n| v : " << v << " : ";
            stack< DirectedEdge > s = sp.getPathTo2( v );
            while ( ! s.empty() ) {
                out << s.top() << ", ";
                s.pop();
            }
        ...
        for ( int v = 0; v < sp.edgeTo.size(); ++v ) {
            out << v << " : ";
            for ( int i = 0; i < sp.edgeTo[ v ].size(); ++i ) {
                out << sp.edgeTo[ v ][ i ] << ", ";
            }
            out << endl;
        }
