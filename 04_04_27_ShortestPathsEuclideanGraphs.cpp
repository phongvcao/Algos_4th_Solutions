//
// Created by Phong Cao on 2019-04-11.
//
...
public:
    ...
    friend bool operator ==( const Point2D& p1, const Point2D& p2 ) {
        return ( abs( p1.x - p2.x ) < numeric_limits< double >::epsilon() )
                && ( abs( p1.y - p2.y ) < numeric_limits< double >::epsilon() );
    }
    ...
    static double getDist( const Point2D& p1, const Point2D& p2 ) {
...
public:
    ...
    void addVertex( int v, Point2D p ) {
        vertices[ v ] = p;
    }

    Point2D getVertex( int v ) const {
        return vertices[ v ];
    }
...
private:
    ...
    vector< bool > shortestDistanceComputed;
    vector< double > finalDistanceTo;
    const EdgeWeightedDigraph* ewDigraph;
    int s = 0;

    void relax( const EdgeWeightedDigraph& G, int v, int t ) {
        double distFromVertexToTarget = Point2D::getDist( G.getVertex( v ), G.getVertex( t ) );

        for ( DirectedEdge e : G.getAdj( v ) ) {
            int w = e.getTo();

            // Euclidean heuristic
            double distFromNeighborToTarget = Point2D::getDist( G.getVertex( w ), G.getVertex( t ) );

            double distToTargetPassingThroughNeighbor = distTo[ v ] + e.getWeight() + distFromNeighborToTarget - distFromVertexToTarget;

            if ( distTo[ w ] > distToTargetPassingThroughNeighbor ) {
                distTo[ w ] = distToTargetPassingThroughNeighbor;
                edgeTo[ w ] = e;
    ...
    void computeSourceSinkShortestPath( int t ) {
        fill( distTo.begin(), distTo.end(), numeric_limits< double >::infinity() );
        DirectedEdge nullEdge;
        fill( edgeTo.begin(), edgeTo.end(), nullEdge );

        double distFromSourceToTarget = Point2D::getDist( ewDigraph->getVertex( s ), ewDigraph->getVertex( t ) );

        distTo[ s ] = distFromSourceToTarget;
        indexMinPQ.insert( s, distFromSourceToTarget );

        while ( ! indexMinPQ.empty() ) {
            int vertexToRelax = indexMinPQ.delMin();
            if ( vertexToRelax == t ) {
                break;
            }
            relax( *ewDigraph, vertexToRelax, t );
        }
        finalDistanceTo[ t ] = distTo[ t ];
    }
...
public:
    ...
    DijkstraSP( const EdgeWeightedDigraph& G, int s )
        : distTo( G.getV(), numeric_limits< double >::infinity() ),
          edgeTo( G.getV() ),
          indexMinPQ( G.getV() ),
          shortestDistanceComputed( G.getV(), false ),
          finalDistanceTo( G.getV(), numeric_limits< double >::infinity() ),
          s( s )
    {
        ewDigraph = &G;
    }
    ...
    bool hasPathTo( int v ) {
        if ( ! shortestDistanceComputed[ v ] ) {
            computeSourceSinkShortestPath( v );
            shortestDistanceComputed[ v ] = true;
        }
        return finalDistanceTo[ v ] != numeric_limits< double >::infinity();
    }
    ...
    double getDistTo( int v ) {
        ...
        return finalDistanceTo[ v ];
    }

    stack< DirectedEdge > getPathTo( int v ) {
        if ( ! shortestDistanceComputed[ v ] ) {
            computeSourceSinkShortestPath( v );
            shortestDistanceComputed[ v ] = true;
        }
    ...
    friend ostream& operator <<( ostream& out, DijkstraSP& sp ) {
        out << "\nsp:\n";
