...
int main( int argc, char ** argv ) {
    ...
    cout << "DEBUG: graph G: \n" << sg.getG()->toString() << endl;

    int s = sg.index( source );
    BFP bfp( *( sg.getG() ), s );
    cout << "DEBUG: bfp: \n" << bfp.toString() << endl;

    /* string firstDest = "Kidman, Nicole";
    cout << firstDest << endl;
    forward_list< int > path = bfp.pathTo( sg.index( firstDest ) );
    for ( int w : path ) {
        cout << "    " << sg.name( w ) << endl;
    } */

    vector< int > baconNum( sg.getG()->getE() );
    int notConnected = 0;
    for ( int i = 0; i < sg.getG()->getV(); ++i ) {
        if ( bfp.hasPathTo( i ) ) {
            int num = 0;
            for ( int w : bfp.pathTo( i ) ) {
                ++num;
            }
            ++baconNum[ num ];
        } else {
    ...
    for ( int i = 0; i < baconNum.size(); ++i ) {
        if ( baconNum[ i ] != 0 ) {
            cout << "i : " << i << " : " << baconNum[ i ] << endl;
        }
    }
    cout << "notConnected : " << notConnected << endl;
