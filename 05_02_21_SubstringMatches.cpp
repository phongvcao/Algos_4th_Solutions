...
private:
    ...
    void getKeysThatContains( Node * h, string prefix, queue< Value >& results ) const {
        if ( ! h ) return;

        getKeysThatContains( h->left, prefix, results );

        if ( h->val ) results.push( *( h->val ) );
        getKeysThatContains( h->mid, prefix + h->c, results );

        getKeysThatContains( h->right, prefix, results );
    }
...
public:
    ...
    queue< Value > getKeysThatContains( const string& prefix ) const {
        queue< Value > results;
        Node * h = get( root, prefix, 0 );
        if ( ! h ) return results;
        if ( h->val ) results.push( *( h->val ) );
        getKeysThatContains( h->mid, prefix, results );
        return results;
    }
...
int main( int argc, char ** argv ) {
    ...
    TST< unordered_set< string > > tst;

    for ( int i = 0; i < strVec.size(); ++i ) {
        for (int j = 0; j < strVec[ i ].size(); j++) {
            unordered_set< string > valVec;
            string key = strVec[ i ].substr( j );
            if ( tst.contains( key ) ) {
                valVec = tst.get( key );
            }
            valVec.insert( strVec[ i ] );
            tst.put( key, valVec );
        }
    }
    ...
    cout << "DEBUG: tst.getKeysThatContains( el ) : ";
    queue< unordered_set< string > > valsQueue = tst.getKeysThatContains( "el" );
    while ( ! valsQueue.empty() ) {
        for ( const string& val : valsQueue.front() ) {
            cout << val << ", ";
        }
        valsQueue.pop();
    }
    ...
    cout << "DEBUG: tst.getKeysThatContains( e ) : ";
    valsQueue = tst.getKeysThatContains( "e" );
    while ( ! valsQueue.empty() ) {
        for ( const string& val : valsQueue.front() ) {
            cout << val << ", ";
        }
    ...
    cout << "DEBUG: tst.getKeysThatContains( z ) : ";
    valsQueue = tst.getKeysThatContains( "z" );
    while ( ! valsQueue.empty() ) {
        for ( const string& val : valsQueue.front() ) {
            cout << val << ", ";
        }
    ...
    cout << "DEBUG: tst.getLongestPrefixOf( hel ) : " << tst.getLongestPrefixOf( "hel" ) << endl;
