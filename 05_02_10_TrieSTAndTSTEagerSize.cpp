...
private:
    ...
    Node * newNode() {
        nodes.push_back( make_unique< Node >( N ) );
    ...
    Node * put( Node * h, string key, Value val, int d ) {
        ...
        char c = key[ d ];
        int nextSize = size( h->next[ c ] );
        h->next[ c ] = put( h->next[ c ], key, val, d + 1 );
        h->N = h->N - nextSize + size( h->next[ c ] );
        return h;
    }
...
private:
    ...
    Node * put( Node * h, string key, Value val, int d ) {
        ...
        h->N = size( h->mid ) + size( h->left ) + size( h->right );
        if ( h->val ) ++h->N;
        return h;
    }
