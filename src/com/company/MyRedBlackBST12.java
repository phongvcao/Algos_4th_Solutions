...
public class MyRedBlackBST12< Key extends Comparable< Key >, Value > {
    ...
    /* public void deleteInBST( Key key ) {
        root = deleteInBST( root, key );
    }

    public Node deleteInBST( Node h, Key key ) {
        if ( h == null ) return null;
        int cmp = key.compareTo( h.key );
        if ( cmp < 0 ) {
            h.left = deleteInBST( h.left, key );
        } else if ( cmp > 0 ) {
            h.right = deleteInBST( h.right, key );
        } else {
    ...
    public static void main( String[] args ) {
        ...
        MyRedBlackBST12< String, String > bst = new MyRedBlackBST12< String, String >();
