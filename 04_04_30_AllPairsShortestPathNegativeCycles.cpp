...
public:
    ...
    friend ostream& operator <<( ostream& out, const IndexMinPQ< Key >& indexMinPQ ) {
        out << "\npq:\n";
        for ( int v = 0; v < indexMinPQ.pq.size(); ++v ) {
            out << v << " : " << indexMinPQ.pq[ v ] << endl;
        }

        out << "\nqp:\n";
        for ( int v = 0; v < indexMinPQ.qp.size(); ++v ) {
            out << v << " : " << indexMinPQ.qp[ v ] << endl;
        }

        out << "\nkeys:\n";
        for ( int v = 0; v < indexMinPQ.keys.size(); ++v ) {
            out << v << " : " << indexMinPQ.keys[ v ] << endl;
        }
...
class AllPairsShortestPathWithoutNegCycles {
private:
...
public:
    ...
    AllPairsShortestPathWithoutNegCycles( const EdgeWeightedDigraph& G )
          ...
          vertexWeights( G.getV(), numeric_limits< double >::infinity() )
    {
        ...
        for ( int v = 0; v < V; ++v ) {
            ...
            for ( int w = 0; w < V; ++w ) {
                ...
                double realWeight = curEdge.getWeight() - vertexWeights[ curFrom ] + vertexWeights[ curTo ];
    ...
    virtual ~AllPairsShortestPathWithoutNegCycles() {
        // not implemented
    }
    ...
    friend ostream& operator <<( ostream& out, const AllPairsShortestPathWithoutNegCycles& sp ) {
        out << "\nsp:\n";
        for ( int v = 0; v < sp.edgeTo.size(); ++v ) {
            out << "\n| v : " << v << " : \n";
...
int main( int argc, char ** argv ) {
    ...
    AllPairsShortestPathWithoutNegCycles sp3( ewDigraph1 );
    cout << "DEBUG: sp3: \n" << sp3 << endl;
