...
class BellmanFordNegCycleHeuristicSP {
private:
...
public:
    ...
    BellmanFordNegCycleHeuristicSP( const EdgeWeightedDigraph& G, int s )
          ...
          onQueue( G.getV(), false )
    {
        ...
        while ( ! relaxQueue.empty() && ! hasNegativeCycle() ) {
            ...
            relaxQueue.pop();
            onQueue[ v ] = false;
            if ( edgeTo[ v ] && onQueue[ edgeTo[ v ].getFrom() ] ) continue;
            relax( G, v );
        }
    }

    virtual ~BellmanFordNegCycleHeuristicSP() {
        // not implemented
    }
    ...
    friend ostream& operator <<( ostream& out, const BellmanFordNegCycleHeuristicSP& sp ) {
        out << "\nsp:\n";
        try {
...
int main( int argc, char ** argv ) {
    ...
    BellmanFordNegCycleHeuristicSP sp( ewDigraph, 0 );
    cout << "DEBUG: sp: \n" << sp << endl;
    ...
    BellmanFordNegCycleHeuristicSP sp1( ewDigraph1, 0 );
    cout << "DEBUG: sp1: \n" << sp1 << endl;
