...
public:
    ...
    friend ostream& operator <<( ostream& out, const WeightedQuickUnionUF& uf ) {
        ...
        out << "\nid:\n";
        for ( int v = 0; v < uf.id.size(); ++v ) {
            out << v << " : " << uf.id[ v ] << endl;
        }

        out << "\nsz:\n";
        for ( int v = 0; v < uf.sz.size(); ++v ) {
            out << v << " : " << uf.sz[ v ] << endl;
        }
        return out;
    }
...
private:
    ...
    vector< bool > marked;
    vector< int > id;
    vector< forward_list< Edge > > comp;
    const EdgeWeightedGraph* ewGraph;
    int count = 0;
...
public:
          ...
          ewGraph( &G )
    {
        for ( int v = 0; v < G.getV(); ++v ) {
            if ( ! marked[ v ] ) {
                forward_list< Edge > compEdges;
                comp.push_back( compEdges );
                dfs( G, v );
                ++count;
            }
        }
    ...
    void dfs( const EdgeWeightedGraph& G, int v ) {
        marked[ v ] = true;
        id[ v ] = count;
        for ( Edge e : G.getAdj( v ) ) {
            int w = e.getOther( v );
            if ( ! marked[ w ] ) {
                comp[ count ].push_front( e );
                dfs( G, w );
            }
        }
    }

    int getCompSize( int compId ) const {
        int size = 1;
        for ( const Edge& e : getComponent( compId ) ) {
            ++size;
        }
        return size;
    }
    ...
    forward_list< Edge > getComponentEdges( int compId ) const {
        return comp[ compId ];
    }

    forward_list< Edge > getComponent( int compId ) const {
        forward_list< Edge > edges = ewGraph->getEdges();
        forward_list< Edge > compEdges;
        for ( Edge e : edges ) {
            int v = e.getEither();
            int w = e.getOther( v );
            if ( ( id[ v ] == id[ w ] ) && ( id[ v ] == compId ) ) {
                compEdges.push_front( e );
            }
        }
        return compEdges;
    }
    ...
    friend ostream& operator <<( ostream& out, const CC& cc ) {
        out << "\ncc: count : " << cc.getCount() << endl;
        out << "\nid:\n";
...
private:
    ...
    vector< bool > marked;
    vector< vector< Edge > > edgeTo;
    vector< double > distTo;
    IndexMinPQ< double > indexMinPQ;

public:

    PrimMSTWithCC( const EdgeWeightedGraph& G, const CC& cc )
        : marked( G.getV(), false ),
          edgeTo( cc.getCount() ),
          ...
          indexMinPQ( G.getV() )
    {
        for ( int i = 0; i < cc.getCount(); ++i ) {
            vector< Edge > newEdgeTo( G.getV() );
            edgeTo[ i ] = newEdgeTo;
            int v = cc.getComponentEdges( i ).front().getEither();
            if ( ! marked[ v ] ) {
                distTo[ v ] = 0;
                indexMinPQ.insert( v, 0 );
                while ( ! indexMinPQ.empty() ) {
                    visit( G, i, indexMinPQ.delMin() );
                }
            }
        }
    }

    virtual ~PrimMSTWithCC() {
        // not implemented
    }

    void visit( const EdgeWeightedGraph& G, int compId, int v ) {
        marked[ v ] = true;
        for ( Edge e : G.getAdj( v ) ) {
            int w = e.getOther( v );
    ...
    queue< Edge > getEdges( int compId ) const {
        queue< Edge > q;
        for ( int v = 0; v < edgeTo[ compId ].size(); ++v ) {
            if ( edgeTo[ compId ][ v ] ) {
                q.push( edgeTo[ compId ][ v ] );
            }
        }
        return q;
    }

    double getWeight( int compId ) const {
        double weight = 0;
        queue< Edge > q = getEdges( compId );
        while ( ! q.empty() ) {
    ...
    friend ostream& operator <<( ostream& out, const PrimMSTWithCC& mst ) {
        out << "\nmst:\n";
        for ( int i = 0; i < mst.edgeTo.size(); ++i ) {
            out << "\ncompId : " << i << " ; weight : " << mst.getWeight( i ) << endl;
            queue< Edge > q = mst.getEdges( i );
            while ( ! q.empty() ) {
                out << q.front() << ", ";
                q.pop();
            }
...
class KruskalMSTWithCC {
private:

    vector< queue< Edge > > mst;

public:

    KruskalMSTWithCC( const EdgeWeightedGraph& G, const CC& cc )
        : mst( cc.getCount() )
    {
        for ( int i = 0; i < cc.getCount(); ++i ) {
            forward_list< Edge > edges = cc.getComponent( i );
            priority_queue< Edge, vector< Edge >, greater< Edge > > minPQ( edges.begin(), edges.end() );
            WeightedQuickUnionUF uf( G.getV() );

            while ( ! minPQ.empty() && mst[ i ].size() < cc.getCompSize( i ) - 1 ) {
                Edge e = minPQ.top();
                minPQ.pop();
                int v = e.getEither();
                int w = e.getOther( v );
    ...
    friend ostream& operator <<( ostream& out, const KruskalMSTWithCC& mst ) {
        out << "\nmst:\n";
        for ( int i = 0; i < mst.mst.size(); ++i ) {
            out << "\ncompId : " << i << " ; weight : " << mst.getWeight( i ) << endl;
            queue< Edge > q = mst.getEdges( i );
            while ( ! q.empty() ) {
                out << q.front() << ", ";
...
int main( int argc, char ** argv ) {
    ...
    PrimMSTWithCC mst1( ewGraph, cc );
    cout << "DEBUG: PrimMSTWithCC: \n" << mst1 << endl;

    KruskalMSTWithCC mst2( ewGraph, cc );
    cout << "DEBUG: KruskalMSTWithCC: \n" << mst2 << endl;
