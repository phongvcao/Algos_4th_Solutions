...
public:
    ...
    Path( double weight, DirectedEdge e )
        : weight( weight ),
          lastEdge( e )
    {
        // not implemented
    ...
    DirectedEdge getLastEdge() const {
        return lastEdge;
    }

};

class DijkstraMonotonicSP {
private:

    vector< double > distTo;
    vector< DirectedEdge > edgeTo;

    vector< double > distToMonoAscending;
    vector< DirectedEdge > edgeToMonoAscending;

    vector< double > distToMonoDescending;
    vector< DirectedEdge > edgeToMonoDescending;

public:

    DijkstraMonotonicSP( const EdgeWeightedDigraph& G, int s )
        : distTo( G.getV(), numeric_limits< double >::infinity() ),
          edgeTo( G.getV() ),
          distToMonoAscending( G.getV(), numeric_limits< double >::infinity() ),
          edgeToMonoAscending( G.getV() ),
          distToMonoDescending( G.getV(), numeric_limits< double >::infinity() ),
          edgeToMonoDescending( G.getV() )
    {
        ...
        while ( ! pathMinPQ.empty() ) {
            ...
            DirectedEdge curEdge = curShortestPath.getLastEdge();
            ...
            while ( ! ascNextVertexInfo->empty() ) {
                ...
                if ( e.getWeight() <= prevEdgeWeight ) {
                    break;
                }

                edgeToMonoAscending[ nextVertex ] = curShortestPath.getLastEdge();
                distToMonoAscending[ nextVertex ] = curShortestPath.getWeight();

                Path path( curShortestPath.getWeight() + e.getWeight(), e );
                pathMinPQ.push( path );
            }

            if ( ! edgeToMonoAscending[ nextVertex ] ) {
                edgeToMonoAscending[ nextVertex ] = curEdge;
                distToMonoAscending[ nextVertex ] = curShortestPath.getWeight();
            }
        }
        ...
        distToMonoDescending[ s ] = 0;
        priority_queue< DirectedEdge, vector< DirectedEdge >, greater< DirectedEdge > > * descSourceVertexInfo = &descVerticesInfo[ s ];
        while ( ! descSourceVertexInfo->empty() ) {
            DirectedEdge e = descSourceVertexInfo->top();
            descSourceVertexInfo->pop();
        ...
        while ( ! pathMinPQ.empty() ) {
            ...
            while ( ! descNextVertexInfo->empty() ) {
                ...
                edgeToMonoDescending[ nextVertex ] = curShortestPath.getLastEdge();
                distToMonoDescending[ nextVertex ] = curShortestPath.getWeight();

                Path path( curShortestPath.getWeight() + e.getWeight(), e );
                pathMinPQ.push( path );
            }

            if ( ! edgeToMonoDescending[ nextVertex ] ) {
                edgeToMonoDescending[ nextVertex ] = curEdge;
                distToMonoDescending[ nextVertex ] = curShortestPath.getWeight();
            }
        }


        // Adjust distTo[] && edgeTo[] accordingly
        for ( int v = 0; v < edgeTo.size(); ++v ) {
            if ( distToMonoAscending[ v ] < distToMonoDescending[ v ] ) {
                distTo[ v ] = distToMonoAscending[ v ];
                edgeTo[ v ] = edgeToMonoAscending[ v ];
            } else {
                distTo[ v ] = distToMonoDescending[ v ];
                edgeTo[ v ] = edgeToMonoDescending[ v ];
            }
        }
    }

    virtual ~DijkstraMonotonicSP() {
        // not implemented
    }
    ...
    friend ostream& operator <<( ostream& out, const DijkstraMonotonicSP& sp ) {
        out << "\nsp:\n";
...
int main( int argc, char ** argv ) {
    ...
    DijkstraMonotonicSP sp( ewDigraph, 0 );
    cout << "DEBUG: sp: \n" << sp << endl;
