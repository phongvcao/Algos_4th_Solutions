...
public:
          ...
          adj( V )
    {
        for ( int v = 0; v < getV(); ++v ) {
            adj[ v ].resize( getV() );
            fill( adj[ v ].begin(), adj[ v ].end(), numeric_limits< double >::infinity() );
        }
    }
    ...
    forward_list< int > getAdj( int v ) const {
        forward_list< int > adjList;
        for ( int w = 0; w < adj[ v ].size(); ++w ) {
            if ( adj[ v ][ w ] != numeric_limits< double >::infinity() ) {
                adjList.push_front( w );
            }
        }
        return adjList;
    }
    ...
    forward_list< Edge > getEdges() const {
        ...
        for ( int v = 0; v < adj.size(); ++v ) {
            for ( int w = 0; w < adj[ v ].size(); ++w ) {
                if ( adj[ v ][ w ] != numeric_limits< double >::infinity() && w > v ) {
                    Edge e( v, w, adj[ v ][ w ] );
                    all.push_front( e );
                }
            }
...
int main( int argc, char ** argv ) {
    ...
    for ( int i = 0; i < 16; ++i ) {
        ewGraph.addEdge( (int)graphArr[ i ][ 0 ], (int)graphArr[ i ][ 1 ], graphArr[ i ][ 2 ] );
    }
