...
public class QuickSortSentinel {
    ...
    public static void sort( Comparable[] a ) {
        ...
        for ( int i = 0; i < a.length; ++i ) {
            if ( less( a[ maxIdx ], a[ i ] ) ) maxIdx = i;
        }
        exchange( a, maxIdx, a.length - 1 );
    ...
    public static int partition( Comparable[] a, int low, int high ) {
        ...
        while ( true ) {
            while ( less( a[ ++i ], pivot ) && less( a[ i ], a[ high ] ) ) StdOut.print( "" );
            while ( less( pivot, a[ --j ] ) ) StdOut.print( "" );
