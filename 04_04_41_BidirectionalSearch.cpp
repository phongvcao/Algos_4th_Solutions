...
public:
    ...
    EdgeWeightedDigraph reverse() const {
        EdgeWeightedDigraph R( getV() );
        for ( DirectedEdge e : getEdges() ) {
            DirectedEdge edge( e.getTo(), e.getFrom(), e.getWeight() );
            R.addEdge( edge );
        }
        return R;
    }
...
class BidirectionalDijkstraSP {
private:

    vector< DirectedEdge > edgeToSource;
    vector< double > distToSource;
    vector< bool > relaxedFromSource;
    
    vector< DirectedEdge > edgeToTarget;
    vector< double > distToTarget;
    vector< bool > relaxedFromTarget;

    IndexMinPQ< double > indexMinPQ;

    void relax( const EdgeWeightedDigraph& G, int v ) {
        if ( v < G.getV() ) {
            // Process normal Graph
            for ( DirectedEdge e : G.getAdj( v ) ) {
                int w = e.getTo();
                if ( distToSource[ w ] > distToSource[ v ] + e.getWeight() ) {
                    distToSource[ w ] = distToSource[ v ] + e.getWeight();
                    edgeToSource[ w ] = e;

                    if ( indexMinPQ.contains( w ) ) {
                        indexMinPQ.change( w, distToSource[ w ] );
                    } else {
                        indexMinPQ.insert( w, distToSource[ w ] );
                    }
                }
            }
            ...
            for ( DirectedEdge e : G.getAdj( v ) ) {
                int w = e.getTo();
                if ( distToTarget[ w ] > distToTarget[ v ] + e.getWeight() ) {
                    distToTarget[ w ] = distToTarget[ v ] + e.getWeight();
                    edgeToTarget[ w ] = e;

                    if ( indexMinPQ.contains( w ) ) {
                        indexMinPQ.change( w, distToTarget[ w ] );
                    } else {
                        indexMinPQ.insert( w, distToTarget[ w ] );
                    }
                }
            }
            relaxedFromTarget[ v ] = true;
        }
    }

public:

    BidirectionalDijkstraSP( const EdgeWeightedDigraph& G, int s, int t )
        : distToSource( G.getV(), numeric_limits< double >::infinity() ),
          edgeToSource( G.getV() ),
          relaxedFromSource( G.getV(), false ),
          distToTarget( G.getV(), numeric_limits< double >::infinity() ),
          edgeToTarget( G.getV() ),
          relaxedFromTarget( G.getV(), false ),
          indexMinPQ( 2 * G.getV() )
    {
        distToSource[ s ] = 0;
        ...
        distToTarget[ t ] = 0;
        indexMinPQ.insert( t + G.getV(), 0 );
        EdgeWeightedDigraph R = G.reverse();

        while ( ! indexMinPQ.empty() ) {
            int nextVertex = indexMinPQ.getMinIndex();
            indexMinPQ.delMin();
            if ( nextVertex < G.getV() ) {
                relax( G, nextVertex );
            } else {
                relax( R, nextVertex );
            }

            if ( relaxedFromSource[ nextVertex ] && relaxedFromTarget[ nextVertex ] ) {
    ...
    virtual ~BidirectionalDijkstraSP() {
        // not implemented
    }

    void computeShortestPath( const EdgeWeightedDigraph& G ) {
        double shortestDist = numeric_limits< double >::infinity();
        int intermediateVertex = -1;

        for ( int v = 0; v < G.getV(); v++ ) {
            if ( distToSource[ v ] + distToTarget[ v ] < shortestDist ) {
                shortestDist = distToSource[ v ] + distToTarget[ v ];
                intermediateVertex = v;
            }
        }

        for ( DirectedEdge e = edgeToTarget[ intermediateVertex ]; e; e = edgeToTarget[ e.getFrom() ] ) {
            distToSource[ e.getFrom() ] = distToSource[ e.getTo() ] + e.getWeight();
            DirectedEdge edge( e.getTo(), e.getFrom(), e.getWeight() );
            edgeToSource[ e.getFrom() ] = edge;
        }
    }
    ...
    stack< DirectedEdge > getPathTo( int v ) const {
        ...
        for ( DirectedEdge e = edgeToSource[ v ]; e; e = edgeToSource[ e.getFrom() ] ) {
            path.push( e );
        }
        return path;
    }
    ...
    bool hasPathTo( int v ) const {
        return distToSource[ v ] != numeric_limits< double >::infinity();
    }
    ...
    friend ostream& operator <<( ostream& out, const BidirectionalDijkstraSP& sp ) {
        out << "\nsp:\n";
        for ( int v = 0; v < sp.edgeToSource.size(); v++ ) {
            out << "\n| v : " << v << " : ";
        ...
        out << "\n\ndistTo:\n";
        for ( int v = 0; v < sp.distToSource.size(); v++ ) {
            out << v << " : " << sp.distToSource[ v ] << endl;
        }

        out << "\n\nedgeTo:\n";
        for ( int v = 0; v < sp.edgeToSource.size(); v++ ) {
            out << v << " : " << sp.edgeToSource[ v ] << endl;
        }
...
int main( int argc, char ** argv ) {
    ...
    DijkstraSP sp1( ewDigraph, 0 );
    cout << "DEBUG: sp1: \n" << sp1 << endl;

    BidirectionalDijkstraSP sp2( ewDigraph, 0, 1 );
    cout << "DEBUG: sp2: \n" << sp2 << endl;
