...
private:
    ...
    bool containsPrefix( Node * h, string prefix ) const {
        if ( ! h ) return false;
        if ( h->val ) return true;

        bool keyFound = false;
        for ( int c = 0; c < R; c++ ) {
            keyFound |= containsPrefix( h->next[ c ], prefix + ( char ) c );
        }
        return keyFound;
    }
...
public:
    ...
    bool containsPrefix( const string& prefix ) const {
        ...
        return containsPrefix( h, prefix );
    }
    ...
    string toString() const {
        ostringstream oss;
        oss << "index : " << root->index << " ; char : '' ; val : " << root->val
            << toString( root ) << endl;
        return oss.str();
    }

    friend ostream& operator <<( ostream& out, const TrieSet& st ) {
        out << "\nst:\n";
...
int main( int argc, char ** argv ) {
    ...
    cout << "DEBUG: trieSet.containsPrefix( shells ) : " << trieSet.containsPrefix( "shells" ) << endl;
    cout << "DEBUG: trieSet.containsPrefix( phong ) : " << trieSet.containsPrefix( "phong" ) << endl;
    cout << "DEBUG: trieSet.containsPrefix( s ) : " << trieSet.containsPrefix( "s" ) << endl;
    cout << "DEBUG: trieSet.containsPrefix( a ) : " << trieSet.containsPrefix( "a" ) << endl;
