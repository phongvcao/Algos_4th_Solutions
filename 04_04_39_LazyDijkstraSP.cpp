...
private:
    ...
    priority_queue< DirectedEdge, vector< DirectedEdge >, greater< DirectedEdge > > minPQ;
    vector< bool > relaxed;
...
public:
    ...
    {
        distTo[ s ] = 0;
        forward_list< DirectedEdge > edges = G.getAdj( s );
        if ( ! relaxed[ s ] ) {
            relax( G, s );
            while ( ! minPQ.empty() ) {
                ...
                int v = e.getFrom();
                int w = e.getTo();
                if ( relaxed[ v ] && relaxed[ w ] ) continue;
                if ( ! relaxed[ v ] ) relax( G, v );
