...
// Implement STInt based on RedBlackBST
class STInt {
private:
...
const bool STInt::RED;
const bool STInt::BLACK;

// Implement STDouble based on RedBlackBST
class STDouble {
private:
    ...
    public:
        ...
        Node( const double& key, int N, int color ) {
            this->key = &key;
            this->N = N;
            this->color = color;
        }
    ...
    NodePtr floor( NodePtr h, const double& key ) {
        if ( ! h ) return NULL;
        if ( key < *( h->key ) ) {
    ...
    NodePtr ceiling( NodePtr h, const double& key ) {
        if ( ! h ) return NULL;
        if ( key > *( h->key ) ) {
...
public:
    ...
    const double* floor( const double& key ) {
    ...
    const double* ceiling( const double& key ) {
...
const bool STDouble::RED;
const bool STDouble::BLACK;
...
int main( int argc, char ** argv ) {
    ...
    // Test STDouble
    STDouble stDouble;
    double keyArr2[] = { 14, 12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9 };
    for ( int i = 0; i < sizeof( keyArr2 ) / sizeof( double ); ++i ) {
        stDouble.put( keyArr2[ i ] );
    }
    ...
    queue< double > q2 = stDouble.getKeys();
